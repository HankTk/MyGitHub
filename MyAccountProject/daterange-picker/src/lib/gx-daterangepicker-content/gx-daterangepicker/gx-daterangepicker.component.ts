...
import {GxDaterangepickerContentComponent} from '../gx-daterangepicker-content.component';
import {ComponentPortal} from '@angular/cdk/portal';
import {take} from 'rxjs/operators';
import {DEFAULT_DATES_RANGES} from './gx-calendar-common/gx-calendar-default-ranges';
...
@Component({
  selector: 'gx-daterangepicker',
  template: '',
  exportAs: 'GxDateRangePickerComponent',
...
export class GxDateRangePickerComponent<D> extends MatDatepicker<D> implements OnInit, AfterViewInit, OnDestroy {
  ...
  )
  {
     ...
     * Because everything is freaking private in material, why would someone use protected anyway.
     */
    this._drDialog = _drDialog;
    this._drDialog.open = <any>function(...args: any[]) {
      if (typeof args[0].createEmbeddedView !== 'function') {
        args[0] = GxDaterangepickerContentComponent;
      }
    ...
    //     if (value) {
    //       value.component = GxDatePickerContentComponent;
    //     }
    //   }
    // })

    this['_openAsPopup'] = () => {
      const portal = new ComponentPortal<GxDaterangepickerContentComponent<D>>(GxDaterangepickerContentComponent,
  ...
  forwardContentValues(instance: GxDaterangepickerContentComponent<D>)
  {
      ...
      this.customRanges = DEFAULT_DATES_RANGES.map(range => {
        return {
          ...
          startDate: range.startDate
            ? this._drDateAdapter.createDate(range.startDate.getFullYear(), range.startDate.getMonth(), range.startDate.getDate())
            : null,
          endDate: range.endDate
            ? this._drDateAdapter.createDate(range.endDate.getFullYear(), range.endDate.getMonth(), range.endDate.getDate())
            : null
        };
      });
    }
