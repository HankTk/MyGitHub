...
import {GxDateRangePickerComponent} from '../gx-daterangepicker.component';
import {Subscription} from 'rxjs';

export const SG_DATEPICKER_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => GxDateRangePickerInputDirective),
  multi: true
};
...
@Directive({
  selector: '[gxDaterangepickerEnd]',
  providers: [
    SG_DATEPICKER_VALUE_ACCESSOR,
    SG_DATEPICKER_VALIDATORS,
    {provide: MAT_INPUT_VALUE_ACCESSOR, useExisting: GxDateRangePickerInputDirective},
  ],
  host: {
  ...
  exportAs: 'GxDateRangePickerInputDirective',
})
export class GxDateRangePickerInputDirective<D> extends MatDatepickerInput<D> implements AfterViewInit, OnDestroy
{
  @Input()
  set gxDaterangepickerEnd(value: GxDateRangePickerComponent<D>)
  {
  ...
  private get _gxDaterangepickerEnd(): GxDateRangePickerComponent<D>
  {
    return <any> this._datepicker;
  }

  private _gxDatepickerSubscription = Subscription.EMPTY;
  ...
  constructor(
    private _drElementRef: ElementRef,
    @Optional() public _drDateAdapter: DateAdapter<D>,
    @Optional()
    @Inject(MAT_DATE_FORMATS) private _drDateFormats: MatDateFormats,
  ...
  ngAfterViewInit()
  {
    ...
    //     if(!this._gxDaterangepickerEnd.applyButton){
  ...
  {
    if (this._datepicker) {
      this._gxDatepickerSubscription = this._gxDaterangepickerEnd._selectedChangedRangeEnd.subscribe((selected: D) => {
        this.value = selected;
      ...
      this._drDatepickerClearSubscription = this._gxDaterangepickerEnd._clearRangeEnd.subscribe(() => {
        this.value = null;
  ...
  ngOnDestroy(): void
  {
    super.ngOnDestroy();
    this.unregister();
    this._gxDatepickerSubscription.unsubscribe();
  ...
  {
    if (this._gxDaterangepickerEnd) {
      this._gxDaterangepickerEnd._unregisterInputRangeEnd(this);
    }
  }
